...
class UvnLogger:
  ...
  def exec_command(self, cmd_args, shell: bool = False, check: bool = False):
    import shlex

    if shell:
      cmd_args = ["sh", f"-{'e' if check else ''}c", *cmd_args]
    self.debug("+ " + " ".join(["{}"] * len(cmd_args)), *(map(shlex.quote, cmd_args)))
